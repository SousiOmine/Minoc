...
export class SessionManager {
  ...
  constructor(configManager: ConfigManager, historyRecorder: HistoryRecorder, progressManager: ProgressManager) {
   ...
   * 対話セッションを開始
   */
  async startChat(model?: string, apiKey?: string): Promise<{ sessionId: string; openaiClient: OpenAIClient }> {
    // 設定を読み込み
    ...
    if (!apiKey && !modelConfig.apiKey) {
      throw new Error('APIキーが設定されていません。--api-key オプションで指定するか、設定ファイルに保存してください。');
    }
